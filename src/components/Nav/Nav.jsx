...
export default function Nav({ yPos, categ, setCateg }) {
    ...
    useEffect(() => {
        (async () => {
            ...
            const newCart = await cart2.reduce(
                (a, b) => a + parseInt(b.quantity),
                0
            );
            const priceTotal = await cart2.reduce(
                (a, b) => a + parseInt(b.quantity * b.price),
                0
            );
            setQuant(newCart);
            setPrice(priceTotal);
        ...
        <>
            ...
            <div className={[style.nav, navPos && style.nav__float].join(' ')}>
                <form className={style.nav__row}>
                    ...
                    <div className={style.nav__row__search}>
                        <input
                            type="text"
                            placeholder="Search..."
                    ...
                    <div className={style.nav__row__icons}>
                        <NavLink
                            to="/cart"
                            className={style.nav__row__icons__link}
                        >
                            <div>${price}</div>
                            <img
                                src={cartImage}
                                alt=""
                            />
                            {quant == 0 && <div className={style.blank}></div>}
                            {quant > 0 && (
                        ...
                        <a
                            href="https://seanengineering.github.io/portfolio/"
                            target="_blank"
                        >
                            <img
                                src={contact}
